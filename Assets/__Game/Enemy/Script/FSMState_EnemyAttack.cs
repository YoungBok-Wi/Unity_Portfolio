...
namespace Game
{
    ...
    public class FSMState_EnemyAttack : FSMState_UnitBase
    {
        ...
        {
            FacePlayer();
            PlayAnim(UnitConst.AnimAttack, false);
            m_AnimLen = AnimLength(UnitConst.AnimAttack, 0.5f);
            m_Timer = 0;
            m_Fired = false;
        }
        protected override FSMState OnUpdate()
        {
            ...
            if (!m_Fired && data.Group == UnitConst.GroupTank)
                Move(Unit.Facing);
        ...
        private void Fire(EnemyTable _data)
        {
            ...
            if (_data.Group == UnitConst.GroupRanged)
            {
                var velocity = new Vector2(Unit.Facing * _data.ProjectileSpeed, 0);
                LocalGameManager.instance.Fire(new SProjectile(Unit, Unit.HitPoint, velocity, damage, 0, _data.Range, 0, 0));
                return;
            }
            float reach = _data.StopDistance + _data.HitboxWidth;
            var center = Unit.HitPoint + Vector2.right * (Unit.Facing * reach * 0.5f);
            HitBox(center, new Vector2(reach, UnitConst.HitBoxHeight), damage, 1);
        }
        #endregion
    }
}
