...
namespace Game
{
    ...
    public class FSMState_EnemyMove : FSMState_UnitBase
    {
        ...
        {
            PlayAnim(UnitConst.AnimMove, true);
            m_RetreatSec = 0;
        }
        protected override FSMState OnUpdate()
        {
            ...
            float dist = DistX;
            int dir = DirToPlayer;
            if (data.Group == UnitConst.GroupRanged)
            {
                if (dist < data.StopDistance * 0.7f)
                {
                    ...
                    if (dist <= data.Range && IsRetreatBlocked())
                        return Parent.GetState(UnitConst.StateAttack);
                    return this;
                }
                m_RetreatSec = 0;
            ...
            if (!battle.RequestMeleeSlot(Unit, side))
            {
                ...
                if (UnitConst.MeleeWaitDistance < dist)
                    Move(dir);
                else
                    FacePlayer();
                return this;
        ...
        {
            return Unit.Physics != null && UnitConst.RetreatBlockSec <= m_RetreatSec && Mathf.Abs(Unit.Physics.Rig.linearVelocity.x) < UnitConst.RetreatBlockSpeed;
        }
        #endregion
    }
}
