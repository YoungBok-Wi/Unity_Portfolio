...
namespace Game
{
    ...
    public class FSMState_BossSkill2 : FSMState_UnitBase
    {
        ...
        protected override void OnStart()
        {
            ...
            int count = Boss.IsEnraged ? data.Skill2EnrageAreaCount : data.Skill2AreaCount;
        ...
        protected override FSMState OnUpdate()
        {
            ...
            if (m_IsTelegraph)
            {
                ...
                m_IsTelegraph = false;
                m_Timer = 0;
                PlayAnim(UnitConst.AnimAttack2, false);
                if (data.AttackType == UnitConst.GroupMelee)
            ...
            if (data.AttackType == UnitConst.GroupMelee)
            {
                Move(m_Dir);
                if (!m_Hit)
                    m_Hit = 0 < HitBox(Unit.HitPoint, new Vector2(1.5f, UnitConst.HitBoxHeight), damage, 1);
                float moved = Mathf.Abs(Unit.transform.position.x - m_StartX);
                if (data.Skill2TriggerDistance * 2f <= moved || 1.5f <= m_Timer)
                {
                    Unit.SetMoveSpeed(Boss.IsEnraged ? data.EnrageMoveSpeed : data.MoveSpeed);
                    return Parent.GetState(UnitConst.StateIdle);
                }
                return this;
            }
            if (!m_Hit && AnimLength(UnitConst.AnimAttack2, 0.6f) * 0.5f <= m_Timer)
            {
                m_Hit = true;
                foreach (var center in m_Areas)
                    HitBox(center, new Vector2(data.Skill2AreaWidth, UnitConst.HitBoxHeight * 2f), damage, 1);
            }
            if (AnimLength(UnitConst.AnimAttack2, 0.6f) <= m_Timer)
                return Parent.GetState(UnitConst.StateIdle);
            return this;
        }
        #endregion
