...
namespace Game
{
    ...
    public class FSMState_BossSkill1 : FSMState_UnitBase
    {
        ...
        protected override void OnStart()
        {
            ...
            var data = Boss.BossData;
            m_Timer = 0;
            m_IsTelegraph = true;
            m_Count = 0;
            m_CountTimer = 0;
            if (data.AttackType == UnitConst.GroupMelee)
                LocalGameManager.instance.ShowTelegraph(new Vector2(AreaCenter(data).x, Unit.transform.position.y), data.Skill1Range, data.Skill1Telegraph);
        }
        protected override FSMState OnUpdate()
        {
            ...
            if (m_IsTelegraph)
            {
                ...
                m_IsTelegraph = false;
                m_Timer = 0;
                PlayAnim(UnitConst.AnimAttack1, false);
                m_AnimLen = AnimLength(UnitConst.AnimAttack1, 0.6f);
            }
            ...
            else
            {
                ...
                if (m_Count < data.Skill1Count && m_CountTimer <= 0)
                {
                    ...
                    LocalGameManager.instance.Fire(new SProjectile(Unit, Unit.HitPoint, velocity, damage, 0, UnitConst.BossProjectileRange, 0, 0));
                }
            }
            if (m_AnimLen <= m_Timer && (data.AttackType == UnitConst.GroupMelee || data.Skill1Count <= m_Count))
                return Parent.GetState(UnitConst.StateIdle);
            return this;
        }
        #endregion
        ...
        {
            return new Vector2(_data.Skill1Range, UnitConst.HitBoxHeight);
        }
        #endregion
    }
}
