...
namespace Game
{
    ...
    public class FSMState_BossIdle : FSMState_UnitBase
    {
        ...
        protected override FSMState OnUpdate()
        {
            ...
            if (!Boss.IsEnraged && Unit.Hp.v <= Unit.MaxHp.v * data.EnrageHpRatio)
                return Parent.GetState(UnitConst.StateEnrage);
            if (!HasPlayer)
                return this;
            ...
            float interval = Boss.IsEnraged ? data.Skill1EnrageInterval : data.Skill1Interval;
            if (m_Timer < interval)
                return this;

            float dist = DistX;
            bool isMelee = data.AttackType == UnitConst.GroupMelee;
            bool skill2Ready = data.Skill2Interval <= m_Skill2Timer;
            if (skill2Ready && (!isMelee || data.Skill2TriggerDistance <= dist))
            {
                m_Skill2Timer = 0;
                return Parent.GetState(UnitConst.StateSkill2);
            }
            if (!isMelee || dist <= data.Skill1Range)
                return Parent.GetState(UnitConst.StateSkill1);
            return Parent.GetState(UnitConst.StateMove);
        }
        #endregion
    }
}
