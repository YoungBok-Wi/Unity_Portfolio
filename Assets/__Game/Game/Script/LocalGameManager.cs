...
namespace Game
{
    /// <summary>씬 전투 판정·풀·연출·능력·일시정지를 관리한다.</summary>
    public class LocalGameManager : LocalManagerBase
    {
        ...
        private readonly Dictionary<string, Sprite> m_UnitIcons = new();
        ...
        public override void OnRegisterObject(ObjectBase _object)
        {
            ...
            if (_object is Object_PlayerBase player)
                SetPlayer(player);
        }
        ...
        private void OnRoomKindChanged(ValueBase _)
        {
            ...
            GameManager.instance.PlayBGM(m_Bgm, isBoss ? TableManager.instance.Const.Battle_BossBgmPitch : 1f);
        }
        #endregion
        ...
        private void CreateUnitPool(GameObject _prefab, int _size, Transform _root)
        {
            ...
            m_UnitIcons.Add(unit.Id, unit.Icon);
        }
        /// <summary>공격 단계에 맞춘 히트 이펙트와 효과음을 재생한다.</summary>
        private void PlayHitEffect(Vector2 _point, AudioClip _clip, int _step)
        {
            int last = m_HitStepScale != null ? m_HitStepScale.Length - 1 : -1;
            int index = 0 <= last ? Mathf.Clamp(_step - 1, 0, last) : 0;
            float scale = 0 <= last ? m_HitStepScale[index] : 1f;
            SpawnEffect(m_HitEffectPrefab, _point, m_HitEffectSec, 1, scale, Color.white);
            SoundManager.instance.PlaySE(_clip);
        }
        /// <summary>이펙트를 위치·방향·배율·색에 맞춰 풀에서 재생한다.</summary>
        private void SpawnEffect(GameObject _prefab, Vector2 _point, float _sec, int _facing, float _scale = 1f, Color? _color = null)
        {
            if (_prefab == null)
                return;
            if (!m_EffectPools.TryGetValue(_prefab, out var pool))
            {
                pool = new ObjectPool(_prefab, m_UnitRoot != null ? m_UnitRoot : transform, GameConst.EffectPoolSize);
            ...
            if (go == null)
            {
                ...
                go = pool.Get();
                if (go == null)
                    throw new InvalidOperationException($"{_prefab.name} 이펙트 풀에서 꺼낼 수 없다 (풀 크기 {GameConst.EffectPoolSize})");
            }
            go.transform.position = _point;
            var baseScale = _prefab.transform.localScale;
            go.transform.localScale = new Vector3(baseScale.x * _scale, baseScale.y * _scale, baseScale.z);
            go.GetComponentsInChildren(s_Sprites);
            foreach (var sr in s_Sprites)
            {
                sr.flipX = _facing < 0;
                sr.color = _color ?? Color.white;
            }
        ...
        private void SpawnCrumbDrops(Object_UnitBase _unit, int _amount)
        {
            ...
            {
                DataManager.instance.AddCrumb(_amount);
                return;
            }
            if (m_CrumbDropPool == null)
                m_CrumbDropPool = new ObjectPool(m_CrumbDropPrefab, m_UnitRoot != null ? m_UnitRoot : transform, GameConst.CrumbDropPoolSize);
            int count = Mathf.Min(_amount, GameConst.CrumbDropMax);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    // 풀 소진 — 즉시 적립
                    DataManager.instance.AddCrumb(value);
                    continue;
                }
                var drop = go.GetComponent<CrumbDrop>();
                if (drop == null)
                    throw new InvalidOperationException($"{m_CrumbDropPrefab.name} 에 CrumbDrop 이 없다");
                var velocity = new Vector2(UnityEngine.Random.Range(-GameConst.CrumbTossSpeedX, GameConst.CrumbTossSpeedX), GameConst.CrumbTossSpeedY);
        ...
        private void ApplyAbility(string _id, AbilityTable _table, int _stack)
        {
            switch (_id)
            {
                ...
                case "HealMacaron":
                    ...
                    break;
                case GameConst.AbilityMultiHit:
                    break;
                default:
        ...
        {
            if (!m_AbilityStacks.TryGetValue(GameConst.AbilityMultiHit, out var stack))
                return 0;
            var table = TableManager.instance.Ability.Data[GameConst.AbilityMultiHit];
        ...
        {
            if (!m_UnitIcons.TryGetValue(_id, out var icon))
                throw new ArgumentException($"등록되지 않은 유닛 ID : {_id}", nameof(_id));
            if (icon == null)
                throw new InvalidOperationException($"{_id} 유닛 아이콘이 배선되지 않았다");
            return icon;
        }
        ...
        public Object_UnitBase SpawnUnit(string _id, Vector2 _pos, float _hpScale, float _atkScale)
        {
            ...
            if (go == null)
            {
                ...
                Debug.Log($"[LocalGameManager] {_id} 풀 소진 — 반납 대기 스폰 (풀 크기 {m_EnemyPoolSize}, 대기 {m_PendingSpawns.Count})");
                return null;
            }
            ...
            if (unit is Object_BossBase boss)
            {
                Boss = boss;
        ...
        {
            if (_target == null || _hit.Attacker == null || _target.IsPlayerSide == _hit.Attacker.IsPlayerSide)
                return false;
            if (_target.IsPlayerSide)
            {
                ...
                bool capped = GameConst.PlayerKnockbackDriftMax <= drift * _hit.Direction;
                _hit.KnockbackDist = capped ? 0f : GameConst.PlayerKnockbackDist;
                _hit.KnockbackTime = GameConst.PlayerKnockbackTime;
            }
            if (!_target.TakeHit(_hit))
                return false;
            PlayHitEffect(_hit.Point, m_SfxHit, _hit.Step);
            if (_hit.IsFinish || _target is Object_BossBase)
                HitStop();
        ...
        public int HitBox(Object_UnitBase _attacker, Vector2 _center, Vector2 _size, int _damage, int _maxHits, float _knockbackDist, float _knockbackTime, bool _isFinish, int _step = 1)
        {
            ...
            foreach (var col in Physics2D.OverlapBoxAll(_center, _size, 0))
            {
                ...
                if (unit == null || unit.IsDead.v || unit.IsPlayerSide == _attacker.IsPlayerSide || m_HitBuffer.Contains(unit))
                    continue;
            ...
            foreach (var unit in m_HitBuffer)
            {
                ...
                if (Hit(new SHit(_attacker, _damage, _knockbackDist, _knockbackTime, _isFinish, dir, unit.HitPoint, _step), unit))
                    hits += 1;
            }
            return hits;
        }
        ...
        public void CollectCrumb(CrumbDrop _drop)
        {
            if (!m_CrumbDrops.Remove(_drop))
                return;
            DataManager.instance.AddCrumb(_drop.Value);
        ...
        public void PlaySlashEffect(Vector2 _center, int _facing, int _step)
        {
            int scaleLast = m_SlashStepScale != null ? m_SlashStepScale.Length - 1 : -1;
            int colorLast = m_SlashStepColor != null ? m_SlashStepColor.Length - 1 : -1;
            float scale = 0 <= scaleLast ? m_SlashStepScale[Mathf.Clamp(_step - 1, 0, scaleLast)] : 1f;
            Color color = 0 <= colorLast ? m_SlashStepColor[Mathf.Clamp(_step - 1, 0, colorLast)] : Color.white;
            SpawnEffect(m_SlashPrefab, _center, m_SlashSec, _facing, scale, color);
        }
        ...
        public int AddAbility(string _id)
        {
            ...
            if (table.StackMode != GameConst.StackInstant)
            {
                stack += 1;
        ...
        public override void MCPDetail(MCPReport _report)
        {
            ...
            _report.Add("bgmPitch", GameManager.instance != null ? GameManager.instance.BgmPitch.ToString("0.00") : "");
        ...
        public override string MCPCheatApply(string _cheatId)
        {
            switch (_cheatId)
            {
                ...
                case "KillPlayer":
                    if (Player != null)
                        Player.TakeHit(new SHit(Boss != null ? (Object_UnitBase)Boss : Player, Player.Hp.v, 0, 0, false, 1, Player.HitPoint));
                    return "{\"success\":true}";
            }
