...
namespace Library
{
    public partial class Table_Wave
    {
        #region Property
        public IReadOnlyDictionary<string, WaveTable> Data => m_Data;
        public IReadOnlyList<string> ID => m_ID;
        public int Count => m_ID.Count;
        #endregion

        #region Value
        private Dictionary<string, WaveTable> m_Data = new();
        ...
        {
            var json = Resources.Load<TextAsset>("Table/TableWave");
            if (json == null) return;
        ...
        public void OnApplyBackend(ContentItem _chart, Table_All _all)
        {
            foreach (JsonData v in _chart.contentJson)
            {
            	...
            	var item = new WaveTable("Wave", key, "", dic);
