...
namespace Library
{
    public partial class Table_Enemy
    {
        #region Property
        public IReadOnlyDictionary<string, EnemyTable> Data => m_Data;
        public IReadOnlyList<string> ID => m_ID;
        public int Count => m_ID.Count;
        #endregion

        #region Value
        private Dictionary<string, EnemyTable> m_Data = new();
        ...
        {
            var json = Resources.Load<TextAsset>("Table/TableEnemy");
            if (json == null) return;
            ...
            foreach (var id in data.Keys)
            {
                ...
                m_Data.Add(id, new("Enemy", id, "", dic));
                m_ID.Add(id);
        ...
        public void OnApplyBackend(ContentItem _chart, Table_All _all)
        {
            foreach (JsonData v in _chart.contentJson)
            {
            	...
            	var item = new EnemyTable("Enemy", key, "", dic);
