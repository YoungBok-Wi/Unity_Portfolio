...
namespace Library
{
    public partial class Table_Room
    {
        #region Property
        public IReadOnlyDictionary<string, RoomTable> Data => m_Data;
        public IReadOnlyList<string> ID => m_ID;
        public int Count => m_ID.Count;
        #endregion

        #region Value
        private Dictionary<string, RoomTable> m_Data = new();
        ...
        {
            var json = Resources.Load<TextAsset>("Table/TableRoom");
            if (json == null) return;
        ...
        public void OnApplyBackend(ContentItem _chart, Table_All _all)
        {
            foreach (JsonData v in _chart.contentJson)
            {
            	...
            	var item = new RoomTable("Room", key, "", dic);
