...
namespace Library
{
    public partial class Table_Text
    {
        #region Property
        public IReadOnlyDictionary<string, TextTable> Data => m_Data;
        public IReadOnlyList<string> ID => m_ID;
        public int Count => m_ID.Count;
        #endregion

        #region Value
        private Dictionary<string, TextTable> m_Data = new();
        ...
        {
            var json = Resources.Load<TextAsset>("Table/TableText");
            if (json == null) return;
        ...
        public void OnApplyBackend(ContentItem _chart, Table_All _all)
        {
            foreach (JsonData v in _chart.contentJson)
            {
            	...
            	var item = new TextTable("Text", key, "", dic);
