...
namespace Library
{
    /// <summary>알파 페이드로 팝업을 여닫는 연출. m_Targets 의 대상들을 같은 커브·시간으로 동시에 페이드하며, 대상마다 타이밍을 다르게 하려면 이 컴포넌트를 여러 개 둔다</summary>
    public class PopupAnimation_Alpha : PopupAnimationBase
    {
        #region Type
        /// <summary>페이드할 대상 하나의 시작·끝 알파</summary>
        [Serializable]
        public class Entry
        {
            [Tooltip("페이드 대상 CanvasGroup")] public CanvasGroup target;
            [Tooltip("열기 시작 알파")] public float openStart = 0f;
            [Tooltip("열기 끝 알파")] public float openEnd = 1f;
            [Tooltip("닫기 시작 알파")] public float closeStart = 1f;
            [Tooltip("닫기 끝 알파")] public float closeEnd = 0f;
        }
        #endregion
        #region Inspector
        [Title("Open")]
        ...
        [Title("Targets (동시 페이드)")]
        [Tooltip("같은 타이밍(위 Open/Close 커브·시간)으로 동시에 페이드할 대상들. 대상마다 시작·끝 알파를 각자 가진다. 다른 타이밍이 필요하면 컴포넌트를 여러 개 둔다")]
        ...
        /// <summary>열기 페이드를 재생한다</summary>
        // 팝업은 timeScale=0 에서도 움직여야 해서 unscaled 시간을 쓴다
        private IEnumerator OpenCoroutine()
        {
            Collect(true, out var groups, out var starts, out var ends);
            for (int i = 0; i < groups.Count; i++)
                groups[i].alpha = starts[i];
            ...
            while (timer < m_OpenTime)
            {
                ...
                for (int i = 0; i < groups.Count; i++)
                    groups[i].alpha = Mathf.LerpUnclamped(starts[i], ends[i], p);
                yield return null;
            }

            for (int i = 0; i < groups.Count; i++)
                groups[i].alpha = ends[i];
            m_OpenCoroutine = null;
            OnEndOpen();
        }
        /// <summary>닫기 페이드를 재생한다</summary>
        private IEnumerator CloseCoroutine()
        {
            Collect(false, out var groups, out var starts, out var ends);
            for (int i = 0; i < groups.Count; i++)
                groups[i].alpha = starts[i];
        ...
        /// <summary>페이드할 대상과 그 시작·끝 알파를 모은다 (배선되지 않은 대상은 건너뛴다)</summary>
        private void Collect(bool _open, out List<CanvasGroup> _groups, out List<float> _starts, out List<float> _ends)
        {
            _groups = new List<CanvasGroup>();
            _starts = new List<float>();
            _ends = new List<float>();
            if (m_Targets == null) return;

            foreach (Entry e in m_Targets)
            {
                if (e == null || e.target == null) continue;
                _groups.Add(e.target);
