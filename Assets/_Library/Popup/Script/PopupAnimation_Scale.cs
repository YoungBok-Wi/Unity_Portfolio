...
namespace Library
{
    /// <summary>크기 변화로 팝업을 여닫는 연출. m_Targets 의 대상들을 같은 커브·시간으로 동시에 움직이며, 대상마다 타이밍을 다르게 하려면 이 컴포넌트를 여러 개 둔다</summary>
    public class PopupAnimation_Scale : PopupAnimationBase
    {
        #region Type
        /// <summary>스케일할 대상 하나의 시작·끝 크기</summary>
        [Serializable]
        public class Entry
        {
            [Tooltip("스케일 대상 Transform")] public Transform target;
            [Tooltip("열기 시작 스케일")] public Vector3 openStart = Vector3.one * 0.01f;
            [Tooltip("열기 끝 스케일")] public Vector3 openEnd = Vector3.one;
            [Tooltip("닫기 시작 스케일")] public Vector3 closeStart = Vector3.one;
            [Tooltip("닫기 끝 스케일")] public Vector3 closeEnd = Vector3.one * 0.01f;
        }
        #endregion
        #region Inspector
        [Title("Open")]
        ...
        [Title("Targets (동시 스케일)")]
        [Tooltip("같은 타이밍(위 Open/Close 커브·시간)으로 동시에 스케일할 대상들. 대상마다 시작·끝 크기를 각자 가진다. 다른 타이밍이 필요하면 컴포넌트를 여러 개 둔다")]
        ...
        private IEnumerator OpenCoroutine()
        {
            ...
            while (timer < m_OpenTime)
            {
                ...
                for (int i = 0; i < targets.Count; i++)
                    targets[i].localScale = Vector3.LerpUnclamped(starts[i], ends[i], p);
                yield return null;
            }

            for (int i = 0; i < targets.Count; i++)
                targets[i].localScale = ends[i];
            m_OpenCoroutine = null;
            OnEndOpen();
        }
        ...
        /// <summary>스케일할 대상과 그 시작·끝 크기를 모은다 (배선되지 않은 대상은 건너뛴다)</summary>
